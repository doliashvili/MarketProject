...
namespace Core.Common
{
    public static class AssemblyScanner
    {
        ...
        public static Dictionary<Type, Type> FindReadModels(params Assembly[] assemblies)
        {
            var readModelDictionary = new Dictionary<Type, Type>();
            ...
            foreach (var assembly in assemblies)
            {
                var types =
                    assembly.GetTypes()
                        .Where(t => t.IsClass
                                    ...
                                    && t.GetInterfaces().Any(
                                        x => x.IsGenericType
                                             && x.GetGenericTypeDefinition() == typeof(IReadModel<>)));
                ...
                foreach (var type in types)
                {
                    ...
                    readModelDictionary.Add(type, interfaceType.GetGenericArguments()[0]);
                }
            }

            return readModelDictionary;
        }
    }
}